...
Sentry の DSN は実質的に「書き込み専用トークン」です。漏洩しても他人のデータは見えませんが、ノイズイベントを送り込まれる可能性があります。

1. Sentry 管理画面で新しい DSN を発行
2. `.env` を更新

   ```env
   SENTRY_LARAVEL_DSN=<new-dsn>      # サーバー側
   VITE_SENTRY_DSN=<new-dsn>          # フロント側
   ```

3. **`VITE_SENTRY_DSN` を変えた場合は `npm run build`**
4. アプリ再起動 + テストエラー送信で確認
...
- [ ] 旧シークレットを発行元（fincode 管理画面、Sentry、SMTP プロバイダ等）で **失効** した
