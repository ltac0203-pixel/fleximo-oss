...
> **重要 — 加盟店契約は「プラットフォームタイプ」が必須**
> Fleximo は1運営者が複数の飲食店テナントの決済を取り次ぐプラットフォーム型ソフトウェアです。
> そのため、fincode 加盟店契約は **「プラットフォームタイプ」** を選択する必要があります。
> 通常加盟店タイプではテナントごとのショップ ID 切り替え（`Tenant-Shop-Id` ヘッダ）が機能せず、
> 各テナントへの売上分配・テナント別の決済が成立しません。
...
1. <https://www.fincode.jp/> でアカウント登録する。**加盟店種別は必ず「プラットフォームタイプ」を選択**する
