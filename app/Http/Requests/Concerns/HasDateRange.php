<?php

declare(strict_types=1);

namespace App\Http\Requests\Concerns;

use Carbon\Carbon;
use Illuminate\Validation\Validator;

trait HasDateRange
{
    /**
     * @return array<string, array<int, string>>
     */
    protected function dateRangeRules(): array
    {
        return [
            'start_date' => ['required', 'date', 'date_format:Y-m-d'],
            'end_date' => ['required', 'date', 'date_format:Y-m-d', 'after_or_equal:start_date'],
        ];
    }

    /**
     * @return array<string, string>
     */
    protected function dateRangeMessages(): array
    {
        return [
            'start_date.required' => '開始日を指定してください。',
            'start_date.date' => '開始日は有効な日付を指定してください。',
            'start_date.date_format' => '開始日はY-m-d形式で指定してください。',
            'end_date.required' => '終了日を指定してください。',
            'end_date.date' => '終了日は有効な日付を指定してください。',
            'end_date.date_format' => '終了日はY-m-d形式で指定してください。',
            'end_date.after_or_equal' => '終了日は開始日以降を指定してください。',
        ];
    }

    protected function dateRangeMaxDaysCallback(int $maxDays = 366): \Closure
    {
        return function (Validator $validator) use ($maxDays): void {
            $startInput = $this->input('start_date');
            $endInput = $this->input('end_date');
            if (! is_string($startInput) || ! is_string($endInput)) {
                return;
            }

            try {
                $start = Carbon::createFromFormat('Y-m-d', $startInput)->startOfDay();
                $end = Carbon::createFromFormat('Y-m-d', $endInput)->startOfDay();
            } catch (\Throwable) {
                return;
            }

            if ($start->diffInDays($end) > $maxDays) {
                $validator->errors()->add('end_date', "期間は{$maxDays}日以内で指定してください。");
            }
        };
    }

    public function getStartDate(): Carbon
    {
        return Carbon::createFromFormat('Y-m-d', $this->validated('start_date'))->startOfDay();
    }

    public function getEndDate(): Carbon
    {
        return Carbon::createFromFormat('Y-m-d', $this->validated('end_date'))->startOfDay();
    }
}
